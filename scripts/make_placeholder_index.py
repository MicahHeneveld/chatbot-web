...
def main() -> None:
    ...
    # The placeholder always uses the local embedder so the index matches the
    # default query-time backend (no fastembed dependency) and is deterministic.
    embedder = retrieval.LocalHashEmbedder()
    chunks = build_chunks()
    ...
    manifest = {
        ...
        "embed_backend": "local",
        "embed_dim": retrieval.LocalHashEmbedder.DIM,
        "git_sha": git_sha(),
